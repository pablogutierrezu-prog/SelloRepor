...
export default function App() {
  ...
  const handleAddNewDac = (newDacData: Partial<DacRequest>) => {
    ...
    const numIds = dacs.map(d => parseInt(d.id.replace(/^L-?/, ''))).filter(id => !isNaN(id));
    const nextId = numIds.length > 0 ? Math.max(...numIds) + 1 : 20260001;

    const isLicitacion = newDacData.type === 'Licitación';
    const newId = isLicitacion ? `L-${nextId}` : nextId.toString();
    ...
    const newDac: DacRequest = {
      ...
      division: newDacData.division || 'Corporativa',
      gerencia: newDacData.gerencia || 'Gerencia de TI',
